...
*/
		...
		filterMainTable = function (Data2,filtIndex2) {
			DataTemp =new Array(filtIndex2.length);
...
functionAtRedraw = function () {
			...
			if ( typeof this != 'undefined' ) {
				...
				//console.log(divsObject)
									...
									//}
				filtIndex = this.fnGetColumnData(0)
				DataTemp2 = filterMainTable(Data,filtIndex)
				//TempoSettings = this.fnSettings();
			 ...
			 hightablesObjRedraw = new HighTables(DataTemp2,NameIndex,ChartColnames,allChartNames,allChartIndicies,allTableNames,allTableIndicies,allChartTypes)

				
			 ChartIndiciesBarTemp = find("barchart",hightablesObjRedraw.chartTypes)
			 ChartIndiciesBubbleTemp = find("bubblechart",hightablesObjRedraw.chartTypes)
			 ChartIndiciesBoxTemp = find("boxplot",hightablesObjRedraw.chartTypes)
			 ChartIndiciesLineTemp = find("linechart",hightablesObjRedraw.chartTypes)

			//for (j=0;j<ChartIndiciesBarTemp.length;j++)
			//{
			//	highChartTemp = make2dArrayForHighChartsBar(hightablesObjRedraw.chartColnames,hightablesObjRedraw.data,hightablesObjRedraw.chartIndicies[ChartIndiciesBarTemp[j]],hightablesObjRedraw.nameIndex)
			//	makeHighChartBar(highChartTemp,hightablesObjRedraw.chartNames[ChartIndiciesBarTemp[j]])
			//}
			
			//for (j=0;j<ChartIndiciesBubble.length;j++)
			//{
			//hichChartTempBubbleByCat = make2dArrayForHighChartsBubbles(hightablesObjRedraw.chartColnames,hightablesObjRedraw.data,hightablesObjRedraw.chartIndicies[ChartIndiciesBubble[j]],hightablesObjRedraw.nameIndex)
			//makeHighChartBubbleByCategory(hichChartTempBubbleByCat,hightablesObjRedraw.chartNames[ChartIndiciesBubble[j]])
			//}


			//for (j=0;j<ChartIndiciesBox.length;j++)
			//{
			//console.log(hightablesObjRedraw.chartIndicies[ChartIndiciesBox[j]])
			//hichChartTempBoxPlot = make2dArrayForHighChartsBoxplot(hightablesObjRedraw.chartColnames,hightablesObjRedraw.data,hightablesObjRedraw.chartIndicies[ChartIndiciesBox[j]],hightablesObjRedraw.nameIndex)
			//console.log(hichChartTempBoxPlot)
			//makeHighChartBoxPlot(hichChartTempBoxPlot,hightablesObjRedraw.chartNames[ChartIndiciesBox[[j]]])
			//}


			//for (j=0;j<ChartIndiciesLine.length;j++)
			//{
			//console.log(hightablesObjRedraw.chartIndicies[ChartIndiciesLine[j]])
			//hichChartTempline = make2dArrayForHighChartsLine(hightablesObjRedraw.chartColnames,hightablesObjRedraw.data,hightablesObjRedraw.chartIndicies[ChartIndiciesLine[j]],hightablesObjRedraw.nameIndex)
			//console.log(hichChartTempline)
			//makeHighChartLine(hichChartTempline,hightablesObjRedraw.chartNames[ChartIndiciesLine[[j]]])
			//}
...
makeDataTable = function (dataTable,Trial2,Trial3,functionAtRedraw,ToFilter) {
	...
	n = TempString.concat(Trial3.replace('#',''),TempString3);
	console.log(n)
	...
	return Tempr
		...
		make2dArrayForDataTable = function (ChartColnames,Data,ChartIndex2,NameIndex) {
										   ...
										   FullRow1=new Array(Data.length);
										   names1 = new Array(Data.length);
										   Tom1 = new Array(Data.length);
										   ChartCategories1 = new Array(ChartIndex2.length);
		...
		make2dArrayForHighChartsBar = function (ChartColnames,Data,ChartIndex,NameIndex) {
									 ...
									 tChartCategories1 = new Array(Data.length);
									 tFullRow = new Array(ChartIndex.length)
									 tnames = new Array(ChartIndex.length);
									 tTom = new Array(ChartIndex.length);
									tNameIndex = [0];
		...
		make2dArrayForHighChartsBoxplot = function (ChartColnames,Data,ChartIndex,NameIndex) {
									 ...
									 tChartCategories1 = new Array(Data.length);
									 tNames = new Array(Data.length);
									 tFullRow = new Array(Data.length);
		...
		make2dArrayForHighChartsBubbles = function (ChartColnames,Data,ChartIndex2,NameIndex) {
										   ...
										   FullSeries =new Array(ChartIndex2.length);
										  //var names1 = new Array(Data.length);
										  //var Tom1 = new Array(Data.length);
										   ChartCategories1 = new Array(Data.length);
