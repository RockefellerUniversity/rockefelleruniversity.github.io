...
$.fn.dataTableExt.oApi.fnFilterAll = function(oSettings, sInput, iColumn, bRegex, bSmart) {
    ...
    for ( var i=0 ; i<settings.length ; i++ ) {
      ...
      console.log(settings[i].oInstance.fnSettings().aaSorting)
      console.log(settings[i].oInstance)
    }
};
...
functionAtRedraw = function () {
			...
			if ( typeof this != 'undefined' ) {
				...
				RNAQCdata2 = filterMainTable(RNAQCdata,filtIndex)
				var oSettings = this.fnSettings();
				//console.log(oSettings.oPreviousSearch.sSearch)
				Bubble2 = make2dArrayForHighChartsBar(chartColnames,RNAQCdata2,[2,3,4],0)
				//console.log(Bubble2)
				makeHighChartBarGrouped(Bubble2,"#container2")
				highChartline2 = make2dArrayForHighChartsLine(chartColnames,RNAQCdata2,IndexForLine,0)
				//console.log(highChartline2)
				makeHighChartLine(highChartline2,"#container3")	
				Bubble4 = make2dArrayForHighChartsBar(chartColnames,RNAQCdata2,[11,12,13,14,15],0)
							//console.log(Bubble)
				makeHighChartBar(Bubble4,"#container4")
			}
	...
	return Tempr
		...
		make2dArrayForHighChartsBar = function (ChartColnames,Data,ChartIndex,NameIndex) {
									...
									for(i=0;i < ChartIndex.length;i++)
									{
										...
										for (k=0;k<Data.length;k++)
										{
											...
											tFullRow[i][k] = parseFloat(Data[k][ChartIndex[i]])
										}
										  ...
										  for (i=0;i<ChartIndex2.length;i++)
										  {
										    ...
										    for(k=0;k<Data.length;k++)
										    {
										    	...
										    	PerValue[0] = Data[k][ChartIndex2[i]]
										  		...
										  		FullRow1[k] = parseFloat(PerValue)
		...
		make2dArrayForHighChartsLine = function (ChartColnames,Data,ChartIndex2,NameIndex) {
										  ...
										  for (i=0;i<Data.length;i++)
										  {
										    ...
										    for(k=0;k<ChartIndex2.length;k++)
										    {
										      FullRow1[i][k] = parseFloat(Data[i][ChartIndex2[k]]) ;
...
makeHighChartBar = function (highChart,Trial){
			$(Trial).highcharts({
				...
				title: {
					text: 'Propotion of Aligned Reads in Intergenic and Genic Regions'
				},
				xAxis: {
				...
				yAxis: {
					min: 0,
					title: {
						text: 'Proportion of aligned reads'
					}
				},
				legend: {
...
makeHighChartBarGrouped = function (highChart,Trial){
			$(Trial).highcharts({
				chart: {
					type: 'bar'
				},
				title: {
					text: 'Number of Total, Ribosomal and Coding Region Reads'
				},
				xAxis: {
					categories: highChart["categories"],
...
makeHighChartLine = function (highChart,Trial){
			$(Trial).highcharts({
				...
				title: {
					text: 'Gene Coverage Normalised Across Length'
				},
				xAxis: {
					categories: highChart["categories"],
					 tickPositions: [0,9,19,29,39,49,59,69,79,89,99],
					title: {
						text: 'Normalised position across gene'
					}
