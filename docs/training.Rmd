...
# The course sheet is maintained by hand in Redmine, so cells pick up stray
# characters. A Mac Roman non-breaking space (0xCA) pasted in front of a URL
# once rendered as href="<ca>https://..." , which browsers then resolved as a
# relative link. Keep only printable ASCII, and emit no anchor for a blank cell.
asLink <- function(url, label){
  # useBytes: the stray bytes make the string invalid UTF-8, which gsub
  # refuses to process in character mode.
  url <- trimws(gsub("[^\x20-\x7E]", "", as.character(url), useBytes = TRUE))
  ifelse(is.na(url) | !nzchar(url), "", paste0("<a href=\"", url, "\">", label, "</a>"))
}
...
out_df %>%
  ...
  mutate(Material=asLink(Material,"Link"),
         SignUp=asLink(SignUp,"Register")) %>%
